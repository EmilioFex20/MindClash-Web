  ...
  <main class="dash">
    ...
    <header class="wrap top">
      <div class="row">
        ...
        <div class="actions">
          <button class="iconBtn" type="button" @click="go('/profile')" aria-label="Profile">
            <Gift class="i" />
          </button>
      ...
      <section class="card xpCard">
        <div class="xpRow">
          ...
          <span class="xpValue">{{ levelInfo.xpWithinLevel }}/ {{ XP_PER_LEVEL }} XP</span>
        </div>
    ...
    <section class="wrap">
      <div class="grid4">
        <button class="card stat" type="button" @click="go('/profile')">
    ...
    <section class="wrap section">
      <div class="sectionHead">
        ...
        <button class="linkBtn" type="button" @click="go('/quizzes')">
          View All <ChevronRight class="mini" />
        </button>
      </div>
      ...
      <div class="stack">
        <article
          ...
          @keydown.enter.prevent="!c.completed && go('/quiz')"
          @keydown.space.prevent="!c.completed && go('/quiz')"
          role="button"
          :tabindex="c.completed ? -1 : 0"
          :aria-disabled="c.completed"
        >
    ...
    <section class="wrap section">
      ...
      <div class="grid2">
        <article
          ...
          @keydown.enter.prevent="goToQuizzes(s.name)"
          @keydown.space.prevent="goToQuizzes(s.name)"
          role="button"
          tabindex="0"
        >
          <div class="subRow">
            <div class="subIcon" :style="{ background: subjectColor(s.id) }">
...
import { computeLevel, XP_PER_LEVEL } from '@/utils/level-utils'
...
const totalXp = computed(() =>
  Math.max(Number(progress.value.totalXp ?? 0), Number(profile.value?.xp ?? 0)),
)
const levelInfo = computed(() => computeLevel(totalXp.value))

const userData = computed(() => ({
  username: 'CodeNinja',
  avatar: '🥷',
  streak: Number(profile.value?.streak ?? 0),
  totalPoints: Number(profile.value?.totalPoints ?? totalXp.value),
  rank: profile.value?.rank ?? '—',
  completedToday: Number(profile.value?.completedToday ?? 0),
}))
...
const xpPercent = computed(() => Math.round(levelInfo.value.progressPct))
