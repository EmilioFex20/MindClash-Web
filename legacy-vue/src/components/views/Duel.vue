...
onMounted(() => {
  const fromParam = String(route.params.matchId ?? '')
  const fromQuery = String(route.query.matchId ?? '')
  const seed = (fromParam || fromQuery || '').trim()
  if (seed) matchId.value = seed
})

const { match, remainingMs } = useMatch(computed(() => matchId.value ?? undefined)) as unknown as {
  match: Ref<DuelMatch | null>
